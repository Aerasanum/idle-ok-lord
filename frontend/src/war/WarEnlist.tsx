  ...
  return (
    <View style={{ gap: 8, marginTop: 8 }} testID="war-enlist">
      <Row style={{ justifyContent: "space-between" }}>
        ...
        <Txt v="small" color={colors.onSurfaceInverse} style={{ fontFamily: fonts.bodyBold }} testID="war-team-power">Potenza squadra {fmt(detail.team_power ?? 0)}</Txt>
        ...
        {Array.from({ length: 10 }).map((_, i) => {
          ...
          return (
            <View key={i} style={{ width: "48%", flexGrow: 1, height: 36, borderRadius: 6, paddingHorizontal: 8, flexDirection: "row", alignItems: "center", gap: 6, borderWidth: 1.5, borderStyle: pid ? "solid" : "dashed", borderColor: isMe ? colors.goldBright : pid ? colors.forest : colors.wood, backgroundColor: pid ? (isMe ? colors.brandPrimary : colors.forest) : "transparent" }} testID={`war-slot-${i + 1}`}>
              ...
              {pid && pl?.war_power != null ? <Txt v="caption" color={isMe ? colors.goldBright : colors.onBrandTertiary} style={{ fontFamily: fonts.bodyBold }} testID={`war-slot-power-${i + 1}`}>⚔ {fmt(pl.war_power)}</Txt> : pid ? <Icon name="shield-sword" size={14} color={isMe ? colors.goldBright : colors.onBrandTertiary} /> : null}
            </View>
          );
        })}
      </View>
      ...
      <Txt v="caption" color={colors.onSurfaceInverse}>Avversari schierati: {other.length}/10 (la loro potenza resta segreta){side === "attack_roster" && w.defender_id ? "" : side === "attack_roster" ? " (guarnigione NPC)" : ""}</Txt>
    </View>
  );
}
