  ...
  return (
    <View style={styles.container} testID="error-fallback">
      <View style={styles.content}>
        <Text accessibilityRole="header" style={styles.title}>Qualcosa è andato storto</Text>
        <Text style={styles.message}>Ricarica l&apos;app per continuare. I progressi sono salvati sul server.</Text>
        ...
        >
          <Text style={styles.buttonText}>Ricarica l&apos;app</Text>
        </Pressable>
        {__DEV__ ? (
          <Pressable onPress={() => setShowDetails((v) => !v)} accessibilityRole="button" hitSlop={8}>
            <Text style={styles.detailsToggle}>{showDetails ? "Nascondi dettagli" : "Mostra dettagli"}</Text>
          </Pressable>
        ) : null}
      </View>
      {__DEV__ && showDetails ? (
