...
export function KingdomScene({ buildings, castleLevel, tier, heraldicColor, onSelect, queued }: { buildings: { key: string; name: string; level: number; unlocked: boolean }[]; castleLevel: number; tier: { tier: number; name: string; walls: string; roofs: string; banners: string; landmark: string; density?: string }; heraldicColor: string; onSelect: (key: string) => void; queued: Record<string, any> }) {
        ...
        return (
          <Pressable key={b.key} testID={`building-${b.key}`} onPress={() => onSelect(b.key)} style={{ position: "absolute", left: width * pos[0] - size / 2, top: h * pos[1] - size / 2, width: size, height: size + 14, alignItems: "center", opacity: locked ? 0.45 : 1 }}>
            <View style={{ width: size, height: size, borderRadius: b.key === "castle" ? 8 : 6, backgroundColor: built ? (b.key === "castle" ? wallColor : colors.woodDark) : "rgba(0,0,0,0.35)", borderWidth: 2, borderColor: locked ? colors.iron : b.key === "castle" ? colors.gold : colors.wood, alignItems: "center", justifyContent: "center", borderStyle: built ? "solid" : "dashed" }}>
              ...
              {b.key === "castle" ? <View style={{ position: "absolute", top: -12, width: 4, height: 16, backgroundColor: heraldicColor }} /> : null}
              {queued[b.key] ? <View style={{ position: "absolute", right: -6, top: -6, backgroundColor: colors.warning, borderRadius: 10, padding: 3 }}><Icon name="hammer" size={10} color={colors.onWarning} /></View> : null}
            </View>
