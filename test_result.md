...
agent_communication:
  ...
  - agent: "main"
    message: "ITERATION 20c: declare-war UX. Cause of the owner's English error: QAT was 'displaced' (lost its home castle in simulated wars) → English 409 message. Fixes: all wars.py fail() messages are now Italian; client parseError maps codes → Italian (IT dict) when the server message looks English; GET /api/wars/map returns attack_state {can_attack, reason, cooldown_ends_at, active_war_id, attackable_node_ids, members}; node sheet shows the reason (testID declare-blocked-reason) and disables declare-war-button, or 'Bersaglio valido'; attackable nodes get a dashed gold border (testID war-attackable-<id>) + legend 'Attaccabile'. QAT displaced_until cleared in DB for owner testing."
  - agent: "main"
    message: "ITERATION 21: (1) Canon v1.6 (scripts/canon_v16.py, hash 645149e6…): Infirmary — per unit type floor(lost*40%) returns 8h after the war. wars.apply_casualties pushes army.infirmary entries {war_id, units, ready_at} and result.casualties[pid].infirmary {units,total,ready_at}; player.settle() credits ready entries lazily (army.units += …, army.infirmary_returned_total). UI: war detail 'war-infirmary' line, Army tab 'army-infirmary' pending line. Rulebook regenerated (11.3 + formulas). (2) 10v10 formation: WarEnlist shows 'La tua formazione in guerra' block (testID war-my-formation: power, deployed units, command) + 'war-edit-formation-button' → /army/formation. Verified via screenshot + live war (23 units queued in infirmary) + DB-forced ready entry (infantry 19→26)."
  - agent: "main"
    message: "ITERATION 22: Canon v1.7 (scripts/canon_v17.py, hash 94125822…): alliances.roles officers_max 2 (leader + 2 officers hold war powers), alliances.rename {leader only, 7-day cooldown}, neutral garrison documented. PATCH /api/alliances/settings accepts name (leader_required / bad_name / rename_cooldown / name_taken, Italian messages); officers_max message Italian. GET /api/wars/map attack_state adds garrison_lane_power (= round(70% of my alliance median war power)) and garrison_pct_of_median; node sheet shows 'Guarnigione: 10 difensori NPC da ~X potenza' (testID garrison-power) for neutral nodes. UI /alliance: settings panel 'Gestione alleanza' with alliance-rename-input/alliance-rename-button (leader), alliance-description-input/alliance-description-save, join-mode buttons in Italian. Verified live: rename OK + cooldown 409, 3rd promote → 409 'Massimo 2 ufficiali'. QAT name/officers restored."
