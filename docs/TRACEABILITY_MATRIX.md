...
| I02 | Canonical loader + fondamenta Mongo | numeri runtime dal JSON (v1.1 baseline + v1.2 live update via `scripts/canon_v12.py`, hash calcolabile `compute_spec_hash`); idempotenza | `app/core/canon.py` (hash/version/check), `app/core/db.py` (indici), `app/core/ledger.py` (chiave idempotente + guard `recent_ledger_keys`) | `GET /canon/static` → `useCanon` | `test_formulas.py::test_canon_validation`, ledger in `test_battle_attempt_claim_idempotent_and_concurrent` | DONE |
...
| I11 | Eventi, dungeon, quest, imprese, codex | evento sempre attivo, energia, track, 6 dungeon, retention | `app/domain/liveops.py`, `app/domain/progress.py`, `app/routers/liveops.py` | `app/events/index.tsx`, `dungeons.tsx`, `quests.tsx`, `achievements.tsx`, `codex.tsx` | `test_events_dungeons_quests` | DONE |
...
| I13 | Guerra 10v10 asincrona + Titan Hunt | mappa 19×19, snapshot al lock, 10 lane, cattura, boss | `app/domain/wars.py` (`generate_map`, `declare`, `set_roster`, `lock_war`, `resolve_lanes`, `resolve_war`, `tick_wars`, `attack_boss`) | `app/alliance/war.tsx`, `src/war/WarMap.tsx` (legenda, colori proprietà), `app/alliance/boss.tsx` | `test_war_full_cycle_deterministic_and_idempotent` (replay deterministico, snapshot unico), `test_titan_hunt`, `test_alliance_wars.py` (E2E live: dichiara→roster→lock→risolvi→cattura, idempotenza tick; scenario `scripts/seed_war_demo.py`) | DONE · verificato E2E (iterazione 6) |
| I15 | Icone oggetti e materiali illustrate | inventario/forgia/loot mostrano oggetti reali (no testo) | `scripts/gen_art.py` (`items`, `resources`) | `src/ui/ItemIcon.tsx`, `src/art/index.ts` (`itemArt`, `resourceArt`), `hero/gear.tsx`, `(tabs)/battle.tsx` (ResultCard), `offline.tsx` | testing agent iterazione 6 (frontend) | DONE |
...
| `dungeons.catalog[*].formula` | 6 formule | `dungeon_rewards` | idem (forge_depths T2 = 130), `test_harvest_caverns_and_training_grounds` |
| `dungeons.soft_haul_hours`, `dungeons.training_recruit_minutes_per_tier` | `(1+0,5·tier)` ore di produzione; `20·tier` minuti di reclutamento | `dungeon_rewards`, `dungeon_troop_quantity` | `test_harvest_caverns_and_training_grounds` |
...
| I16 | v1.2 contro-unità + rampa difficoltà + rinomina Ariete d'Assedio | tabella approvata dal proprietario: +30%/−20% vs 7 classi; PvE mix stage; guerra mix avversario; enemy_power ×(1+0.004·max(0,s−50)) | `app/domain/formulas.py` (`difficulty_ramp`, `stage_enemy_mix`, `army_class_mix`, `unit_counter_pct`), `player.py` (`army_power`/`combat_profile` con `enemy_mix`), `campaign.py` (attempt), `wars.py` (`lane_power`), `routers/game.py` (`/army` counters, `/battle/stage`) | `(tabs)/army.tsx` (Forte/Debole, bonus regione), `(tabs)/battle.tsx` (Contro-unità), `src/war/WarReplay.tsx` (% per corsia) | `test_formulas.py` (`test_enemy_power_table`, `test_unit_counters_v12`, `test_war_lane_counters_v12`), `test_liveops_social.py` (corsie con counter fields), `test_canon_v12_and_alerts.py` (live) | DONE |
| I17 | Avvisi chat guerra/Titano + replay guerra animato + vetrina Titan Hunt | messaggi di sistema in canale alleanza; 10 duelli animati; Titano 3D + classifica live | `app/domain/social.py` (`alliance_alert`), `wars.py` (declare/roster/lock/cancel/resolve/Titan → alert; `titan_family`, `boss_leaderboard`) | `alliance/chat.tsx` (card evidenziate), `src/war/WarReplay.tsx`, `src/titan/TitanStage.tsx`, `alliance/boss.tsx` | `test_liveops_social.py` (alert declare/roster/lock/VITTORIA), testing agent iterazione 7 | DONE |
| I18 | VFX battaglia v1.2 + solo musica + mura perimetrali | 5 mosse Lord, SUPER ogni 20s, 7 mosse nemici, combo; SFX rimossi; anello mura con torri | — | `src/battle/BattleScene.tsx`, `effects.tsx`, `monsters.tsx`, `lord.tsx`, `src/audio/*`, `src/kingdom/KingdomScene.tsx` (`WallRing`) | testing agent iterazione 7 (frontend) | DONE |
